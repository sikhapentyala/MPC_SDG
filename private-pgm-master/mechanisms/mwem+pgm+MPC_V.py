...
def mwem_pgm(domain, epsilon, delta=0.0, workload=None, rounds=None, maxsize_mb = 25, pgm_iters=1000, noise='gaussian', bounded=False, alpha=0.9):
    ...
    for i in range(1, rounds+1):
        ...
        mpc = MPCComputations_VP(workload_domain_size, est_ans, alice.workload_answers, bob.workload_answers,workload_ids_to_be_computed_private,_column_ids,domain_bins)
        # have workload tuples as indices
        # candidates can be a bit vector of selected marginals here
        ax_index = mpc.select_marginal_worst_approximated(candidates_indices,exp_eps,alice.data,bob.data)
