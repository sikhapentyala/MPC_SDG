...
def mwem_pgm(domain, epsilon, delta=0.0, workload=None, rounds=None, maxsize_mb = 25, pgm_iters=1000, noise='gaussian', bounded=False, alpha=0.9):
    ...
    print(non_private_workload_ids)
    print(_column_ids)
    print(domain_bins)
    ...
    for i in range(1, rounds+1):
        ...
        with open(PATH_MPC+"mwem_V-" + str(max_domain_size) + "-" + str(num_of_workloads) + "-" + str(alice.data.shape[0]) + "-" + str(alice.data.shape[1]) + "-" + str(bob.data.shape[1]), 'w') as outfile:
