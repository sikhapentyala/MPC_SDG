...
def downward_closure(Ws):
    ...
    return list(sorted(ans, key=lambda x: (len(x), sorted(x))))
...
class AIM(Mechanism):
    ...
    def run(self, domain, W):
        ...
        while not terminate:
            ...
            #result_string = '-'.join(str(element) for element in workload_ids_to_be_computed_private)

            with open(PATH_MPC+"Programs/Public-Input/aim_V-" + str(max_domain_size) + "-" + str(num_of_candidates) + "-" + str(alice.data.shape[0]) + "-" + str(alice.data.shape[1]) + "-" + str(bob.data.shape[1]), 'w') as outfile:
...
def default_params():
    ...
    params['domain'] = '../data/diabetes-domain.json'
    params['epsilon'] = 1.0
    params['delta'] = 1e-9
...
if __name__ == "__main__":
    ...
    alice = VDataHolder("Alice", '../data/diabetes_train_alice_v.csv',"vertical",n=0.5)
    bob = VDataHolder("Bob", '../data/diabetes_train_bob_v.csv',"vertical",n=0.5)
    args.save = '../syn_data/compass_aim_V-'+str(args.epsilon)+'_0.csv'
    start_time = time.time()
